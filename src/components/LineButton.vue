<template>
  <a
    ...
    class="inline-flex items-center gap-2 rounded-full bg-[#06C755] px-5 py-3 font-display text-sm font-semibold text-white transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-95"
    :class="floating
      ? 'fixed bottom-5 right-5 z-40 shadow-[0_14px_34px_-10px_rgba(6,199,85,0.7)] hover:-translate-y-0.5 sm:bottom-6 sm:right-6'
  ...
  >
    ...
    <span :class="{ 'hidden sm:inline': floating }">LINE 詢問</span>
  </a>
</template>
...
const url = computed(() => settingsStore.lineUrl)
</script>
